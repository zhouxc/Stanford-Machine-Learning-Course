...
def evaluate_classification(cvf):
	...
	for inst in cvf.listInstTest:
		if inst.fLabel == cvf.classify(dt , inst):
...
def build_pruned_tree(listInstTrain, listInstValidate):
	...
	pruned_tree(dt , listInstValidate)
	return dt
...
def yield_cv_folds_with_validation(listInst, cFold):
	...
	for i in range(cFold - 1):
		listInstTest = folds[i]
		listInstValidation = folds[i + 1]
		listInstTraining = []
		for j in range(cFold):
			if i == j or i + 1 == j:	continue
			listInstTraining += folds[j]

		yield TreeFold(listInstTraining , listInstTest , listInstValidation)
